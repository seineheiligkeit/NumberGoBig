...
//   --amp-base S amplifierBaseRateScale override (fuel-economy lever)
//   --overpay P  fuelOverpayExp override (fuel-economy lever)
...
/** Fuel pick under the diminishing-overpay law: the SMALLEST in-band block whose
 *  effective contribution (`grade^(1-p)·V^p` — the engine's exact formula)
 *  one-shots the remaining work; if none can, the BIGGEST in band (max progress
 *  per action). At p=1 effective = V, so this degrades to "smallest that covers,
 *  else biggest in band" — right-sized fuel is optimal in every regime. */
function takeFuelSmart(w: World, grade: Decimal, cap: Decimal, remaining: Decimal, p: number): Value | null {
  let oneIdx = -1, oneMag: Decimal | null = null;
  let bigIdx = -1, bigMag: Decimal | null = null;
  for (let i = 0; i < w.pool.length; i++) {
    const m = mag(w.pool[i].value);
    if (m.lt(grade) || m.gt(cap)) continue;
    const eff = p >= 1 ? m : grade.pow(1 - p).mul(m.pow(p));
    if (eff.gte(remaining) && (oneMag === null || m.lt(oneMag))) { oneIdx = i; oneMag = m; }
    if (bigMag === null || m.gt(bigMag)) { bigIdx = i; bigMag = m; }
  }
  const bi = oneIdx >= 0 ? oneIdx : bigIdx;
  if (bi < 0) return null;
  const b = w.pool[bi];
  if ((b.count ?? 1) > 1) { b.count -= 1; return b.value; }
  return w.pool.splice(bi, 1)[0].value;
...
const MAX_FUEL_TREES = 8; // two kinds now compete for the bound (moderate-feed + deep-fuel trees)
...
function runSession(rate: number, hours: number, trace: boolean, stacking: boolean, tuning: TimeTuning): {
  ...
  /** Depth whose root one-shots a frontier op under the overpay law:
   *  effective(V) = grade^(1-p)·V^p ≥ W  →  V ≥ (W/grade^(1-p))^(1/p), and the
   *  tree root is 2^(2^depth). (manager.ts's smartFuelDepth, made overpay-aware:
   *  at p<1 the same op needs a far DEEPER denomination.) Clamped [3,6]. */
  function smartTreeDepth(): number {
    const fr = frontier();
    const dig = fr > 1 ? Math.log10(fr) : 1;
    const W = Math.pow(dig + 3, 3); // ~the next frontier op's work (×≤1000 growth)
    const grade = Math.max(1, tuning.gradeCoeff * Math.pow(W, tuning.gradeExp));
    const p = Math.min(1, Math.max(0.05, tuning.fuelOverpayExp));
    const needV = Math.pow(W / Math.pow(grade, 1 - p), 1 / p);
    const depth = Math.ceil(Math.log2(Math.max(2, Math.log2(Math.max(2, needV)))));
    return Math.max(3, Math.min(6, depth));
  }

  /** Queue a balanced multiplication tree → graded fuel, piped, auto-running.
   *  2^depth successors (1s) → 2^depth adders (2s) → squaring mults → 2^(2^depth)
   *  root output. Depth 3 (the opening tree) emits 256s; expansions deepen with
   *  the frontier so one root block keeps one-shotting a frontier op. */
  function queueFuelTree(depth = 3): void {
    fuelTrees++;
    const leaves = 1 << depth;
    const baseY = yOff; yOff += 120 + leaves * 28;
    ...
    build(depth, 0, 'root'); // root spills graded fuel into the loose pool (no output pipe)
  }
  ...
  // Two distinct production pressures, each driving its own bounded expansion:
  //   feedStarve — frontier mults idle for OPERANDS (op0/op1 missing) → the
  //                moderate-block supply is short → build another depth-3 tree
  //                (its 256-root spills are the op1 feedstock).
  //   fuelStarve — working ops find no usable fuel, or only HOMEOPATHIC fuel
  //                (<5% of the op per block) → the fuel GRADE is short → build
  //                a deeper tree (root one-shots a frontier op).
  let feedStarve = 0;
  let fuelStarve = 0;
  let deepestBuilt = 3; // the opening tree's depth — proactive deepening tracks this
    ...
    for (const fm of fmults) {
      ...
      if (c.op === null) {
        ...
        if (c.operands[0] === null) {
          ...
          feedStarve++; // idle mult, nothing to feed — production pressure
          ...
          feedStarve++; // no moderate op1 in stock — production pressure
        }
      } else {
        // Working: pour in graded fuel to rush the digits³ op. Take from the
        // BAND [grade, fuelCap]. The cap admits exactly the fuel grades we
        // PRODUCE (up to the deepest tree root built) and nothing bigger — mid
        // and near-frontier RESULTS stay out of band, reserved as op0/op1
        // feeds. (Burning results as fuel torches the operand supply: the
        // climb grows by ×op1 per op, so a result is worth far more multiplied
        // than burned.)
        const grade = c.op.grade;
        const rootDenom = Decimal.pow(2, Math.pow(2, deepestBuilt));
        const fuelCap = Decimal.max(new Decimal(4096), Decimal.max(grade.mul(64), rootDenom.mul(2)));
        const remaining = c.op.work.sub(c.op.progress);
        const f = takeFuelSmart(world, grade, fuelCap, remaining, tuning.fuelOverpayExp);
        if (f) {
          budget -= 1; counts.fuel++;
          const fv = mag(f);
          const p = tuning.fuelOverpayExp;
          const eff = p >= 1 ? fv : grade.pow(1 - p).mul(fv.pow(p));
          // HOMEOPATHIC fuel (a block buying <5% of the op) is still spent, but
          // it is a supply-GRADE failure: count it as pressure so the agent
          // builds a deeper tree instead of shovelling 256s into a 10⁶-work op.
          // Compare against the op's FULL work, not the remaining slice — a
          // late-op top-up with a tiny block is still the wrong supply grade.
          if (eff.mul(20).lt(c.op.work)) fuelStarve++;
          else fuelStarve = Math.max(0, fuelStarve - 3);
          injectFuel(world, fm, f);
          return true;
        }
        fuelStarve++; // wanted to fuel, but nothing in-band — real fuel pressure
      }
    }

    // 3) Bounded expansion: under sustained pressure, scale the matching kind
    //    of production (a competent player reads WHICH shortage they have).
    //    Operand pressure → another depth-3 tree (moderate-block feedstock);
    //    fuel-grade pressure → a deeper tree (roots one-shot frontier ops).
    if (queue.length === 0 && fuelTrees < MAX_FUEL_TREES) {
      // PROACTIVE deepening (the manager's lesson): when the frontier has
      // outgrown the deepest fuel grade we produce, build the deeper tree as
      // soon as there's mild evidence fuel is binding — don't wait for a full
      // starve (the recycling conveyor masks it). The evidence gate keeps
      // idle-rate runs from sinking their few actions into a 95-cell build.
      const want = smartTreeDepth();
      if (want > deepestBuilt && fuelStarve > 30) {
        deepestBuilt = want;
        fuelStarve = 0;
        queueFuelTree(want);
...
function main(): void {
  ...
  let ampBase = DEFAULT_TUNING.amplifierBaseRateScale, overpay = DEFAULT_TUNING.fuelOverpayExp;
  ...
  console.log(`tuning: amplifierBaseRateScale ${ampBase} · fuelOverpayExp ${overpay}\n`);
  ...
  for (const r of rates) {
    const res = runSession(r, hours, trace && rates.length === 1, stacking, tuning);
